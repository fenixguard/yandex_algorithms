...
O(n^2) - мы проходим по всем документам, а внутри по всем словам документа (n - количество документов).
...
Аналогичная ситуация с построением структуры для подсчета релевантных документов. O(m^2) - где m - количество запросов.
Также мы сортируем эту структура используя встроенную функцию sorted(), которая основана на алгоритме Timsort, отсюда
худшая сложность будет O(k*log(k)), в лучшем O(k), где k - количество элементов структуры содержащей релевантные элементы.
---------------------------------
Пространственная:
---------------------------------
...
2. Поиск релевантных документов:
O(n), где n - количество документов подходящих под запрос на каждом этапе.
Дополнительно - O(m), где m - количество уникальных слов в запросе, мы делаем из листа сет, а также O(k) - для выдачи
